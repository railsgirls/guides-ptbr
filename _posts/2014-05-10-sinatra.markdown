...
Isso irá mostrar a variável `$cafes` no HTML do seu browser para podermos dizer se os café estão sendo adicionados a sua variável $cafes (i.e. que o seu código está funcionando).
...
Como você viu na seção de salvar cafés, se você atualizar o seu browser, o seu app adiciona a informação na variável global `$cafes` repetidamente. Isso acontece porque estamos guardando os parâmetros da URL cada vez que o nosso código `get` é requisitado.
